...
# Συνάρτηση που υπολογίζει το πολυώνυμο 4ου βαθμού με τη μέθοδο ελαχίστων τετραγώνων
def Fourth_Degree_Polynomial(i, x):
    sol = np.linalg.solve(ATA(), C(i))
    y = float(sol[0] + sol[1] * x + sol[2] * x ** 2 + sol[3] * x ** 3 + sol[4] * x ** 4)
    return y
